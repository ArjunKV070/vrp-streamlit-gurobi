...
def plot_routes(initial_routes, final_routes, customer_locations):
    ...
    for v in initial_routes.keys():
        x_aco = [customer_locations.get(i, (0, 0))[0] for i in initial_routes[v]]  # Default to (0, 0) if location missing
        y_aco = [customer_locations.get(i, (0, 0))[1] for i in initial_routes[v]]
    ...
    ax1.scatter(customer_locations.get(0, (0, 0))[0], customer_locations.get(0, (0, 0))[1], c='k', marker='s', s=100, label="Depot")
    ...
    for v in final_routes.keys():
        x_gurobi = [customer_locations.get(i, (0, 0))[0] for i in final_routes[v]]
        y_gurobi = [customer_locations.get(i, (0, 0))[1] for i in final_routes[v]]
    ...
    ax2.scatter(customer_locations.get(0, (0, 0))[0], customer_locations.get(0, (0, 0))[1], c='k', marker='s', s=100, label="Depot")
...
# Function to parse manual distance matrix input
def parse_distance_matrix(input_text):
    """Parse a manually entered distance matrix from text."""
    try:
        lines = [line.strip() for line in input_text.split('\n') if line.strip()]
        matrix = []
        for line in lines:
            # Split by spaces or commas, convert to float
            row = [float(x) for x in line.replace(',', ' ').split()]
            matrix.append(row)
        return np.array(matrix)
    except ValueError as e:
        st.error(f"Invalid distance matrix format: {e}. Ensure numbers are separated by spaces or commas.")
        return None

# Function to solve VRP with ACO and Gurobi
def solve_vrp(distance_matrix, num_vehicles, capacities, costs_per_km, demand_list, customer_locations, plot_enabled):
    """
...
def main():
    ...
    # Option to choose distance matrix input method
    distance_input_method = st.radio("Select Distance Matrix Input Method", ["Upload Excel File", "Manual Entry"])

    # Handle distance matrix input
    if distance_input_method == "Upload Excel File":
        ...
        manual_distance_input = st.text_area("Enter Distance Matrix (one row per line, numbers separated by spaces or commas):\nExample:\n0 10 20\n10 0 30\n20 30 0")
        distance_matrix = parse_distance_matrix(manual_distance_input)
        if distance_matrix is None:
            return

    # Optional location file upload
    use_location_file = st.checkbox("Upload Customer Locations (optional)", value=True)
    if use_location_file:
        ...
        if location_file is not None:
            ...
            st.warning("Please upload the customer locations Excel file or uncheck the option.")
            return
    else:
        customer_locations = {i: (0, 0) for i in range(len(distance_matrix))}  # Default to (0, 0) if no file
        st.write("Using default coordinates (0, 0) for all locations since no file uploaded.")

    # Optional plotting
    plot_enabled = st.checkbox("Show Route Plots", value=True)
    ...
    if st.button("Optimize Routes"):
        ...
        initial_routes, final_routes, total_distance_aco, total_cost_aco, total_distance_gurobi, total_cost_gurobi = solve_vrp(
            distance_matrix, num_vehicles, capacities, costs_per_km, demand_list, customer_locations, plot_enabled
        )
...
# Function to parse manual distance matrix input (defined at the end to avoid forward reference)
def parse_distance_matrix(input_text):
    """Parse a manually entered distance matrix from text."""
    try:
        lines = [line.strip() for line in input_text.split('\n') if line.strip()]
